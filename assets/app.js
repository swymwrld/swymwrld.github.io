...
(async function () {
  ...
  async function initThree() {
    ...
    // Scroll-based parallax for mobile (replacing gyroscope)
    document.addEventListener('scroll', function() {
      if (window.innerWidth <= 768) {
        var maxScroll = Math.max(1, document.body.scrollHeight - window.innerHeight);
        var scrollRatio = Math.min(1, Math.max(0, window.scrollY / maxScroll));
        // Map 0 -> -0.8 (top), 1 -> +0.8 (bottom)
        targetY = (scrollRatio - 0.5) * 1.6;
        // Optional: add slight X variation based on scroll
        targetX = Math.sin(scrollRatio * Math.PI) * 0.4;
      }
    }, { passive: true });
  ...
  function openProject(id) {
    ...
    var isStudio = p.title && p.title.toLowerCase().includes('creative studio');
    var btnHtml = '';
    if (isStudio) {
      btnHtml = '<a class="btn btn-primary" href="#studio" onclick="document.getElementById(\'project-dialog\').close()">Open Studio Section &#8594;</a>';
    } else if (p.url) {
      btnHtml = '<a class="btn btn-primary" target="_blank" rel="noreferrer" href="' + esc(p.url) + '">Open Project &#8594;</a>';
    }
  ...
  function bindSwipe(stageId, onPrev, onNext) {
    ...
    stage.addEventListener('touchmove', function(e) {
      ...
      if (Math.abs(dy) > Math.abs(dx)) isSwiping = false; // Cancel if scrolling vertically
    }, {passive: true});
  ...
  function renderCarousel() {
    carouselVideos = shuffleArray((data.videos || []).slice());
    ...
    bindSwipe('carousel-track', prevSlide, nextSlide);
  }
  ...
  function renderModels() {
    modelItems = shuffleArray((data.models || []).slice());
